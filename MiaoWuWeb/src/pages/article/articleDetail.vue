  ...
  <el-row v-loading="loading">
    <el-col :span="24">
      <div style="text-align: left;">
        <el-button type="text" icon="el-icon-back" @click="goBack" style="padding-bottom: 0px; margin-bottom: 10px">返回</el-button>
      </div>
    </el-col>
    <el-col :span="24">
      <div>
...
<!--          <span style="color: #20a0ff;margin-right:20px;font-size: 12px;">浏览 {{article.pageView==null?0:article.pageView}}</span>-->
          ...
          <el-tag type="success" v-for="(item,index) in article.labels" :key="index" size="small"
                  style="margin-left: 8px">{{item.name}}
          </el-tag>
    ...
    <el-col>
      <div id="content" style="text-align: left"  v-html="article.source">
      </div>
    </el-col>
  </el-row>
    </el-main>
  <el-footer style="">
    ...
    <div id="comment-container" v-for="(comment,index) in article.commentExtends" :key="index">
      <div style="text-align: left; margin-bottom: 5px"> {{comment.user.name}} <span style="color: #20a0ff;margin-right:20px;font-size: 12px;"> {{new Date(comment.date).getTime() | parseTime('{y}-{m}-{d} {h}:{i}')}}</span></div>
        <div style="text-align: left"> {{comment.commentContent}}
      </div>
    </div>
    <el-input
      type="textarea"
...
<script>
  ...
  export default {
    ...
    methods: {
      getArticle() {
        let self = this;
        let url = settings.apiUrl + "article/cascadeFindById" + "?articleId=" + self.$route.query.articleId;
        request.request({
          url,
          method: "get",
          headers: {
